...
/** Run with autoSteer pre-wired (the original helper). */
async function runWithAutoSteer(extra: Partial<LiveConfig>): Promise<BeamOutput[]> {
...
/** Run with NO autoSteer — multiBeam must bring its own DOA (A1 fix). */
async function runMultiBeamOnly(extra: Partial<LiveConfig>): Promise<BeamOutput[]> {
...
describe('LiveEngine multi-beam mode', () => {
  it('absent multiBeam emits no multiBeam field (byte-identical)', async () => {
    const outs = await runWithAutoSteer({});
  ...
  it('multiBeam mode runs, emits mono + slots/gates, and tracks the source', async () => {
    const outs = await runWithAutoSteer({ multiBeam: { nBeams: 3 } });
  ...
  it('TEST-6: multiBeam without autoSteer brings its own DOA — slots update and produce non-silent output (A1 fix)', async () => {
    // Before the A1 fix: multiBeam without autoSteer would never run the DOA cycle, so slots stayed idle
    // and the output was silent all-zeros. After the fix: cov+detect runs, slots update, real output.
    const outs = await runMultiBeamOnly({ multiBeam: { nBeams: 3, detectionHops: 2 } });
    expect(outs.length).toBeGreaterThan(0);
    const last = outs.at(-1)!;
    expect(last.multiBeam).toBeDefined();
    expect(last.multiBeam!.slots.length).toBe(3);
    expect(last.multiBeam!.gates.length).toBe(3);
    expect(Number.isFinite(last.rmsDb)).toBe(true);
    // At least one slot must have received a detection (azimuthDeg is non-null) — proves DOA cycle ran.
    const anySlotFilled = outs.some((o) => o.multiBeam!.slots.some((s) => s.azimuthDeg !== null));
    expect(anySlotFilled).toBe(true);
  });
});
